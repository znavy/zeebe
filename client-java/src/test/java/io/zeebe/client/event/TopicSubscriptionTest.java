...
public class TopicSubscriptionTest
{
    ...
    public void shouldResumeSubscriptionBeforeFailedEventAfterHandlerFailure()
    {
        ...
        final FailingHandler handler = new FailingHandler(e -> e.getMetadata().getEventPosition() == 2L);
        final TopicSubscription subscription = clientRule.topics().newSubscription(clientRule.getDefaultTopicName())
                .startAtHeadOfTopic()
                .handler(handler)
                .name(SUBSCRIPTION_NAME)
    ...
    public void shouldContinueEventHandlingAfterSuccessfulRetry()
    {
        ...
        final FailingHandler handler = new FailingHandler(e ->
                e.getMetadata().getEventPosition() == 1L &&
            ...
            TopicEventType expectedType)
    {
        ...
        assertThat(metadata.getEventKey()).isEqualTo(expectedKey);
        assertThat(metadata.getEventPosition()).isEqualTo(expectedPosition);
        assertThat(metadata.getEventType()).isEqualTo(expectedType);
